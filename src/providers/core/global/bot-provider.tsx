...
import { railway, success, map, tap, bind, logger } from '@/utils'
...
export const BotProvider = ({ children }: { children: React.ReactNode }) => {
  const [botDetectionResult, setBotDetectionResult] =
    useState<BotDetectionResult | null>(null);
...
export const useBotDetection = () => {
  ...
  const log = logger.getSubLogger({ prefix: ['providers', 'core', 'global', 'bot-provider', 'useBotDetection'] })
  if (!context) {
